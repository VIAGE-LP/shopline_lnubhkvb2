const countdown = ()=>{
  const now = new Date();
  //倒數計時開始時間
  const target_start = new Date('2024/05/13 00:00:00'); 
  const differ_start = target_start.getTime() - now.getTime();
  //倒數計時開始區間
  const target_mid_start = new Date('2024/05/13 00:00:00'); 
  const differ_mid_start = target_mid_start.getTime() - now.getTime();
  //倒數計時結束區間
  const target_mid_end = new Date('2024/05/14 00:00:00'); //秒倒數開始
  const differ_mid_end = target_mid_end.getTime() - now.getTime();
  //倒數計時結束時間
  const target_end = new Date('2024/05/14 23:59:59'); //全部結束時間
